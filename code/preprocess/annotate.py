...
def _update_progress_index(progress_index, progress_file_path):
...
def collect_fpkm_annotations(input_xlsx,
    ...
    data_addons = _get_data_addons(df)
    ...
    for i, accession_number in enumerate(df.loc[progress_index:, "accession_number"], start=progress_index):
        ...
        if i % autosave_interval_count == 1:
            _update_data_addons(data_addons)
            _update_progress_index(i, progress_file_path)
        
        sys.stdout.write("\033[K") #clear line 
        
    
    _update_data_addons(data_addons)
    _update_progress_index(0, progress_file_path)
...
def update_fpkm_annotations(input_xlsx,
    ...
    data_addons = _get_data_addons(df_input)
